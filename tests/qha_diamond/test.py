...
from quippy import HBAR, BOLTZMANN_K, GPA
...
try:
   ...
   opt = BFGS
   b=elasticity.fit_elastic_constants(a,symmetry='cubic',optimizer=opt)
...
for dc in [-c,c]:
    atd = bulk.copy()
    atd.set_cell(bulk.get_cell()*(1+dc),scale_atoms=True)
...
for t in temperature:
    ...
    thermal_expansion.append(np.sum(c*mode_gruneisens)/(3.0*bulk.get_volume()*bulk_modulus))
